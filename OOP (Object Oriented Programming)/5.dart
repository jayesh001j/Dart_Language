  ...
  int selectAction(int state) {
    if (Random().nextDouble() < explorationRate) {
      ...
      return qTable
          ...
          .indexOf(qTable.sublist(startIndex, endIndex).reduce(max));
    }
  }

  void learn(int state, int action, int reward, int nextState) {
    int index = state * actionSpaceSize + action;
    double maxQValue = qTable
        .sublist(nextState * actionSpaceSize, (nextState + 1) * actionSpaceSize)
        .reduce(max);
    qTable[index] +=
        learningRate * (reward + discountFactor * maxQValue - qTable[index]);
  }
}
    ...
    for (int episode = 0; episode < numEpisodes; episode++) {
      ...
      while (true) {
        ...
        if (episode == numEpisodes - 1) {
          print(
            'Episode: $episode, State: $state, Action: $action, Reward: $reward',
          );
        }
...
void main() {
  ...
  ReinforcementLearningSystem rlSystem = ReinforcementLearningSystem(
    environment,
    agent,
  );
