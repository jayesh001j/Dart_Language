...
class ComputerVisionSystem {
  ...
  Image applyFilter(Image image, Filter filter) {
    ...
    List<List<int>> newPixels = List.generate(
      imageHeight,
      (_) => List.filled(imageWidth, 0),
    );
      ...
      for (int x = padding; x < imageWidth - padding; x++) {
        ...
        for (int i = 0; i < filterSize; i++) {
          for (int j = 0; j < filterSize; j++) {
            newValue +=
                image.pixels[y - padding + i][x - padding + j] *
                filter.kernel[i][j];
          }
        }
  ...
  Image detectEdges(Image image) {
    ...
    List<List<int>> combinedPixels = List.generate(image.pixels.length, (
      index,
    ) {
    ...
    for (int y = 0; y < image.pixels.length; y++) {
      for (int x = 0; x < image.pixels[0].length; x++) {
        combinedPixels[y][x] = sqrt(
          pow(edgeX.pixels[y][x], 2) + pow(edgeY.pixels[y][x], 2),
        ).round().clamp(0, 255);
      }
    }
